...
#define NTL_WordVectorInputBlock 50
    ...
    public:
        ...
        WordVector(WordVector &x, INIT_TRANS_TYPE) {
            rep = x.rep;
            x.rep = 0;
        }
        ...
        long MaxLength() const { return (!rep) ? 0 : long(rep[-2] >> 1); }
    ...
    inline void swap(WordVector &x, WordVector &y) { x.swap(y); }

    inline void append(WordVector &v, _ntl_ulong a) { v.append(a); }

    inline void append(WordVector &v, const WordVector &w) { v.append(w); }
