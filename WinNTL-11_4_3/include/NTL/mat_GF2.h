...
// some backward compaitibilty stuff
    ...
    inline mat_GF2 to_mat_GF2(const vec_vec_GF2 &a) {
        mat_GF2 x;
        conv(x, a);
        NTL_OPT_RETURN(mat_GF2, x);
    }


    void add(mat_GF2 &X, const mat_GF2 &A, const mat_GF2 &B);

    inline void sub(mat_GF2 &X, const mat_GF2 &A, const mat_GF2 &B) { add(X, A, B); }

    inline void negate(mat_GF2 &X, const mat_GF2 &A) { X = A; }
    ...
    inline void mul(mat_GF2 &X, GF2 a, const mat_GF2 &B) { mul(X, B, a); }

    inline void mul(mat_GF2 &X, const mat_GF2 &A, long b) { mul(X, A, to_GF2(b)); }

    inline void mul(mat_GF2 &X, long a, const mat_GF2 &B) { mul(X, B, a); }
    ...
    inline void sqr(mat_GF2 &X, const mat_GF2 &A) { mul(X, A, A); }
    ...
    inline void power(mat_GF2 &X, const mat_GF2 &A, long e) { power(X, A, ZZ_expo(e)); }
...
// operator notation:
    ...
    inline mat_GF2 operator-(const mat_GF2 &a) { return a; }
