...
inline long &_ntl_ALLOC(_ntl_gbigint p) { return p->alloc_; }

inline long &_ntl_SIZE(_ntl_gbigint p) { return p->size_; }

inline long _ntl_ZEROP(_ntl_gbigint p) {
    return !p || !_ntl_SIZE(p);
}

inline long _ntl_PINNED(_ntl_gbigint p) { return p && (_ntl_ALLOC(p) & 1); }
