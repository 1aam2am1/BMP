...
// we use the specialized memory management routines, if any
        ...
        template<class T>
        static void do_BlockDestroy(T *p, long n) { BlockDestroy(p, n); }

        template<class T>
        static void do_BlockConstruct(T *p, long n) { BlockConstruct(p, n); }

        template<class T>
        static void do_BlockConstructFromVec(T *p, long n, const T *q) { BlockConstructFromVec(p, n, q); }

        template<class T>
        static void do_BlockConstructFromObj(T *p, long n, const T &q) { BlockConstructFromObj(p, n, q); }

    };

    template<>
    struct VecStrategy<false> {
...
// they are defined
        ...
        template<class T>
        static void do_BlockDestroy(T *p, long n) { default_BlockDestroy(p, n); }

        template<class T>
        static void do_BlockConstruct(T *p, long n) { default_BlockConstruct(p, n); }

        template<class T>
        static void do_BlockConstructFromVec(T *p, long n, const T *q) { default_BlockConstructFromVec(p, n, q); }


        template<class T>
        static void do_BlockConstructFromObj(T *p, long n, const T &q) { default_BlockConstructFromObj(p, n, q); }
    ...
    private:
        ...
        static void BlockDestroy(T *p, long n) { VecStrategy<NTL_RELOC_TAG>::do_BlockDestroy(p, n); }

        static void BlockConstruct(T *p, long n) { VecStrategy<NTL_RELOC_TAG>::do_BlockConstruct(p, n); }
    ...
    public:
        ...
        Vec(Vec &x, INIT_TRANS_TYPE) { _vec__rep.swap(x._vec__rep); }
...
// Some compatibility with vec_GF2
        ...
        const T &get(long i) const { return (*this)[i]; }

        void put(long i, const T &a) { (*this)[i] = a; }
...
// EXCEPTIONS: provides strong ES
    ...
    template<class T>
    long operator!=(const Vec<T> &a, const Vec<T> &b) { return !(a == b); }
