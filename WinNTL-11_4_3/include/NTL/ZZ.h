...
// The value then becomes 0.
        ...
        void swap(ZZ &x) { _ntl_gswap(&rep, &x.rep); }
...
// returns max allocation request, possibly rounded up a bit...
        ...
        long SinglePrecision() const { return _ntl_gsptest(rep); }

// tests if less than NTL_SP_BOUND in absolute value

        long WideSinglePrecision() const { return _ntl_gwsptest(rep); }
...
// swap the values of x and y (swaps pointers only)
    ...
    inline double log(const ZZ &a) { return _ntl_glog(a.rep); }
...
***********************************************************/
    ...
    inline void conv(unsigned int &x, const ZZ &a) { x = (unsigned int) (_ntl_gtouint(a.rep)); }

    inline unsigned int to_uint(const ZZ &a) { return (unsigned int) (_ntl_gtouint(a.rep)); }
    ...
    inline void ZZFromBytes(ZZ &x, const unsigned char *p, long n) { _ntl_gfrombytes(&x.rep, p, n); }
    ...
    inline void BytesFromZZ(unsigned char *p, const ZZ &a, long n) { _ntl_gbytesfromz(p, a.rep, n); }
...
// zero test
    ...
    inline long IsOne(const ZZ &a) { return _ntl_gisone(a.rep); }
// test for 1


/* the usual comparison operators */

    inline long operator==(const ZZ &a, const ZZ &b) { return _ntl_gcompare(a.rep, b.rep) == 0; }
...
/* single-precision versions of the above */
    ...
    inline void add(ZZ &x, const ZZ &a, long b) { _ntl_gsadd(a.rep, b, &x.rep); }

    inline void add(ZZ &x, long a, const ZZ &b) { add(x, b, a); }


    inline void sub(ZZ &x, const ZZ &a, long b) { _ntl_gssub(a.rep, b, &x.rep); }
...
/* single-precision versions */
    ...
    inline void mul(ZZ &x, const ZZ &a, long b) { _ntl_gsmul(a.rep, b, &x.rep); }

    inline void mul(ZZ &x, long a, const ZZ &b) { mul(x, b, a); }
...
/* non-standard single-precision versions */
    ...
    inline long DivRem(ZZ &q, const ZZ &a, long b) { return _ntl_gsdiv(a.rep, b, &q.rep); }

    inline long rem(const ZZ &a, long b) { return _ntl_gsmod(a.rep, b); }


/* single precision versions */

    inline void div(ZZ &q, const ZZ &a, long b) { (void) _ntl_gsdiv(a.rep, b, &q.rep); }
...
// if b | a, returns 1; otherwise returns 0.
    ...
    inline long operator%(const ZZ &a, long b) { return rem(a, b); }
...
// compatability.
    ...
    inline long digit(const ZZ &a, long k) { return _ntl_gdigit(a.rep, k); }
...
// x = |a| AND |b|
    ...
    inline void bit_and(ZZ &x, long a, const ZZ &b) { bit_and(x, b, a); }
...
// x = |a| OR |b|
    ...
    inline void bit_or(ZZ &x, long a, const ZZ &b) { bit_or(x, b, a); }
...
// x = |a| XOR |b|
    ...
    inline void bit_xor(ZZ &x, long a, const ZZ &b) { bit_xor(x, b, a); }
    ...
    inline
    long NumBits(long a) { return _ntl_g2logs(a); }
...
// returns least nonnegative k such that 2^k >= m
    ...
    inline
    long NumBytes(const ZZ &a) { return (NumBits(a) + 7) / 8; }

    inline
    long NumBytes(long a) { return (NumBits(a) + 7) / 8; }
...
************************************************************/
    ...
    inline long ZZ_BlockConstructAlloc(ZZ &x, long d, long n) { return _ntl_gblock_construct_alloc(&x.rep, d, n); }

    inline void ZZ_BlockConstructSet(ZZ &x, ZZ &y, long i) { _ntl_gblock_construct_set(x.rep, &y.rep, i); }

    inline long ZZ_BlockDestroy(ZZ &x) { return _ntl_gblock_destroy(x.rep); }

    inline long ZZ_storage(long d) { return _ntl_gblock_storage(d); }
...
//   has changed, otherwise 0
    ...
    inline long CRTInRange(const ZZ &gg, const ZZ &aa) { return _ntl_gcrtinrange(gg.rep, aa.rep); }
...
*************************************************************/
    ...
    inline void power(ZZ &x, const ZZ &a, long e) { _ntl_gexp(a.rep, e, &x.rep); }
    ...
    inline void power(ZZ &x, long a, long e) { _ntl_gexps(a, e, &x.rep); }
...
// x = -a % n
    ...
    inline void AddMod(ZZ &x, long a, const ZZ &b, const ZZ &n) { AddMod(x, b, a, n); }
...
// x = (a*b)%n
    ...
    inline void MulMod(ZZ &x, long a, const ZZ &b, const ZZ &n) { MulMod(x, b, a, n); }
...
// defined in ZZ.c in terms of LowLevelPowerMod
    ...
    inline void PowerMod(ZZ &x, const ZZ &a, long e, const ZZ &n) { PowerMod(x, a, ZZ_expo(e), n); }
