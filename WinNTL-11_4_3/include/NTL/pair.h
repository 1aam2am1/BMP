...
NTL_OPEN_NNS
    ...
    template<class S, class T>
    NTL_DECLARE_RELOCATABLE_WHEN((Pair<S, T> * )) {
...
// FIXME: remove CV-qualifiers and S and T? 
    ...
    template<class S, class T>
    inline long operator==(const Pair<S, T> &x, const Pair<S, T> &y) { return x.a == y.a && x.b == y.b; }

    template<class S, class T>
    inline long operator!=(const Pair<S, T> &x, const Pair<S, T> &y) { return !(x == y); }
