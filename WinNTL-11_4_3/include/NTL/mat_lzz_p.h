...
NTL_OPEN_NNS
    ...
    inline void mul(mat_zz_p &X, zz_p a, const mat_zz_p &B) { mul(X, B, a); }

    inline void mul(mat_zz_p &X, long a, const mat_zz_p &B) { mul(X, B, a); }
...
// ***********************
    ...
    inline void solve(zz_p &d, vec_zz_p &x, const mat_zz_p &A, const vec_zz_p &b) { relaxed_solve(d, x, A, b, false); }

    inline void solve(zz_p &d, const mat_zz_p &A, vec_zz_p &x, const vec_zz_p &b) { relaxed_solve(d, A, x, b, false); }

    inline void inv(zz_p &d, mat_zz_p &X, const mat_zz_p &A) { relaxed_inv(d, X, A, false); }

    inline void inv(mat_zz_p &X, const mat_zz_p &A) { relaxed_inv(X, A, false); }

    inline mat_zz_p inv(const mat_zz_p &A) { return relaxed_inv(A, false); }

    inline void determinant(zz_p &d, const mat_zz_p &A) { relaxed_determinant(d, A, false); }

    inline zz_p determinant(const mat_zz_p &a) { return relaxed_determinant(a, false); }

    inline void power(mat_zz_p &X, const mat_zz_p &A, const ZZ &e) { relaxed_power(X, A, e, false); }

    inline mat_zz_p power(const mat_zz_p &A, const ZZ &e) { return relaxed_power(A, e, false); }

    inline void power(mat_zz_p &X, const mat_zz_p &A, long e) { relaxed_power(X, A, e, false); }

    inline mat_zz_p power(const mat_zz_p &A, long e) { return relaxed_power(A, e, false); }

// ************************


    inline void sqr(mat_zz_p &X, const mat_zz_p &A) { mul(X, A, A); }
