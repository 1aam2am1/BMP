...
// makes x monic
    ...
    inline void reverse(zz_pX &c, const zz_pX &a) { reverse(c, a, deg(a)); }
    ...
    inline void VectorCopy(vec_zz_p &x, const zz_pX &a, long n) { VectorCopy(x, a.rep, n); }

    inline vec_zz_p VectorCopy(const zz_pX &a, long n) { return VectorCopy(a.rep, n); }
...
/* additional legacy conversions for v6 conversion regime */
    ...
    inline void conv(zz_pX &x, const zz_pX &a) { x = a; }

    inline void conv(vec_zz_p &x, const zz_pX &a) { x = a.rep; }
...
**************************************************************/
    ...
    inline long operator!=(const zz_pX &a, const zz_pX &b) { return !(a == b); }
...
// FFT representation of polynomials
    ...
    public:
        ...
        fftRep(const fftRep &R) : k(-1), MaxK(-1), len(0), NumPrimes(0) { *this = R; }

        fftRep(INIT_SIZE_TYPE, long InitK) : k(-1), MaxK(-1), len(0), NumPrimes(0) { SetSize(InitK); }
    ...
    inline void TofftRep_trunc(fftRep &y, const zz_pX &x, long k, long len) { TofftRep_trunc(y, x, k, len, 0, deg(x)); }
...
// computes an n = 2^k point convolution of x[lo..hi].
    ...
    inline void TofftRep(fftRep &y, const zz_pX &x, long k) { TofftRep(y, x, k, 0, deg(x)); }
...
// q = a/b
    ...
    inline void div(zz_pX &q, const zz_pX &a, long b) { div(q, a, to_zz_p(b)); }
...
// x = a^e % f, e >= 0
    ...
    inline void PowerMod(zz_pX &x, const zz_pX &a, long e, const zz_pXModulus &F) { PowerMod(x, a, ZZ_expo(e), F); }
...
// x = X^e % f, e >= 0
    ...
    inline void PowerXMod(zz_pX &x, long e, const zz_pXModulus &F) { PowerXMod(x, ZZ_expo(e), F); }
...
// x = (X + a)^e % f, e >= 0
    ...
    inline zz_pX PowerXPlusAMod(zz_p a, const ZZ &e, const zz_pXModulus &F) {
        zz_pX x;
        PowerXPlusAMod(x, a, e, F);
        NTL_OPT_RETURN(zz_pX, x);
    }

    inline void PowerXPlusAMod(zz_pX &x, zz_p a, long e, const zz_pXModulus &F) { PowerXPlusAMod(x, a, ZZ_expo(e), F); }
...
// same as above, but uses a pre-computed zz_pXArgument
    ...
    inline void project(zz_p &x, const vec_zz_p &a, const zz_pX &b) { InnerProduct(x, a, b.rep); }
...
// required: a.length() >= 2*m
    ...
    inline void ProbMinPolyMod(zz_pX &h, const zz_pX &g, const zz_pXModulus &F) { ProbMinPolyMod(h, g, F, F.n); }
...
// probability at most m/p.
    ...
    inline void MinPolyMod(zz_pX &h, const zz_pX &g, const zz_pXModulus &F) { MinPolyMod(h, g, F, F.n); }
...
// same as above, but guarantees that result is correct
    ...
    inline void IrredPolyMod(zz_pX &h, const zz_pX &g, const zz_pXModulus &F) { IrredPolyMod(h, g, F, F.n); }
