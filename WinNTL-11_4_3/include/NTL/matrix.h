...
NTL_OPEN_NNS
    ...
    public:
        ...
        const_reference operator()(long i, long j) const { return _mat__rep[i - 1][j - 1]; }
    ...
    template<class T>
    NTL_DECLARE_RELOCATABLE((Mat<T> * ))

    template<class T>
    inline const Vec <Vec<T>> &rep(const Mat<T> &a) { return a._mat__rep; }
