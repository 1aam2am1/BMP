        ...
        if (InnerProd_L_viable(l, p)) {
            ...
            NTL_GEXEC_RANGE(seq, n, first, last){
                                                    ...
                                                    for (long i = first; i < last; i++) {
                                                        xp[i].LoopHole() = InnerProd_L(A[i].elts(), bp, l, p,
                                                                                       red_struct, bound);
                                                    }
            ...
            NTL_GEXEC_RANGE(seq, n, first, last){
                                                    ...
                                                    for (long i = first; i < last; i++) {
                                                        xp[i].LoopHole() = InnerProd_LL(A[i].elts(), bp, l, p,
                                                                                        ll_red_struct);
                                                    }
...
//#define DO_MUL(a, b) ((unsigned long) (long(a)*long(b)))
    ...
    static inline
    unsigned long
    DO_MUL(unsigned long a, unsigned long b) { return a * b; }
...
// experiment with shorter int's
    ...
    static inline
    void
    muladd_all_by_32(long first, long last, unsigned long *x, const unsigned int *a, const unsigned int *b, long n) {
        if (n == MAT_BLK_SZ) {
            for (long i = first; i < last; i++)
    ...
    static inline
    void muladd_all_by_32_width(long first, long last, unsigned long *x, const unsigned long *a, const unsigned long *b,
                                long n, long width) {
        if (width == MAT_BLK_SZ) {
            if (n == MAT_BLK_SZ) {
                for (long i = first; i < last; i++)
    ...
    static inline
    void
    muladd_all_by_32(long first, long last, unsigned long *x, const unsigned long *a, const unsigned long *b, long n) {
        if (n == MAT_BLK_SZ) {
            for (long i = first; i < last; i++)
                muladd1_by_32_full(x + i * MAT_BLK_SZ, a + i * MAT_BLK_SZ, b);
        } else {
    ...
    static inline
    void muladd_all_by_32_width(long first, long last, unsigned long *x, const unsigned int *a, const unsigned int *b,
                                long n, long width) {
        if (width == MAT_BLK_SZ) {
            if (n == MAT_BLK_SZ) {
                for (long i = first; i < last; i++)
   ...
   d.LoopHole() = det;
    ...
    void blk_tri_L(zz_p &d, const mat_zz_p &A, const vec_zz_p *bp,
        ...
        for (long kk = 0, kpanel = 0; kk < n; kk += MAT_BLK_SZ, kpanel++) {
            ...
            bool seq =
                    double(npanels - (kpanel + 1)) * double(n) * double(MAT_BLK_SZ) * double(MAT_BLK_SZ) < PAR_THRESH;
            NTL_GEXEC_RANGE(seq, npanels - (kpanel + 1), first, last)
                            NTL_IMPORT(p)
                            NTL_IMPORT(n)
                            NTL_IMPORT(red_struct)
    ...
    long elim_blk_L(const mat_zz_p &A, mat_zz_p *im, mat_zz_p *ker,
        ...
        while (k < w) {
            ...
            if (r > rr) {
                ...
                bool seq =
                        double(npanels - (kpanel + 1)) * double(n - rr) * double(r - rr) * double(MAT_BLK_SZ) <
                        PAR_THRESH;
                ...
                NTL_GEXEC_RANGE(seq, end_block - start_block, first, last)
                                ...
                                for (long index = first; index < last; index++) {
                                    ...
                                    for (long hb = hblocks - 2; hb >= 0; hb--) {
                                        ...
                                        unsigned long ured_trigger =
                                                (~(0UL) - cast_unsigned(p - 1)) /
                                                (cast_unsigned(p - 1) * cast_unsigned(p - 1));
                                        // NOTE: corner case at p == 2: need unsigned long to prevent overflow
