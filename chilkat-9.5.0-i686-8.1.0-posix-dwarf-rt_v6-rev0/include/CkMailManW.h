...
public:
    ...
    bool QuickSend(const wchar_t *fromAddr, const wchar_t *toAddr, const wchar_t *subject, const wchar_t *body,
                   const wchar_t *smtpServer);

    // Creates an asynchronous task to call the QuickSend method with the arguments
    // provided. (Async methods are available starting in Chilkat v9.5.0.52.)
    // The caller is responsible for deleting the object returned by this method.
    CkTaskW *QuickSendAsync(const wchar_t *fromAddr, const wchar_t *toAddr, const wchar_t *subject, const wchar_t *body,
                            const wchar_t *smtpServer);
