...
CkZip_setProgressInfo(HCkZip cHandle, void (*fnProgressInfo)(const char *name, const char *value));
...
CK_C_VISIBLE_PUBLIC void
CkZip_setProgressInfo2(HCkZip cHandle, void (*fnProgressInfo2)(const char *name, const char *value, void *pContext));

CK_C_VISIBLE_PUBLIC void
CkZip_setTaskCompleted2(HCkZip cHandle, void (*fnTaskCompleted2)(HCkTask hTask, void *pContext));
...
CkZip_AddEmbedded(HCkZip cHandle, const char *exeFilename, const char *resourceName, const char *zipFilename);

#endif

CK_C_VISIBLE_PUBLIC void CkZip_AddNoCompressExtension(HCkZip cHandle, const char *fileExtension);

CK_C_VISIBLE_PUBLIC HCkZipEntry
CkZip_AppendBase64(HCkZip cHandle, const char *fileName, const char *encodedCompressedData);
...
CK_C_VISIBLE_PUBLIC HCkZipEntry
CkZip_AppendDataEncoded(HCkZip cHandle, const char *filename, const char *encoding, const char *data);
...
CK_C_VISIBLE_PUBLIC BOOL
CkZip_AppendFilesEx(HCkZip cHandle, const char *filePattern, BOOL recurse, BOOL saveExtraPath, BOOL archiveOnly,
                    BOOL includeHidden, BOOL includeSystem);

CK_C_VISIBLE_PUBLIC HCkTask
CkZip_AppendFilesExAsync(HCkZip cHandle, const char *filePattern, BOOL recurse, BOOL saveExtraPath, BOOL archiveOnly,
                         BOOL includeHidden, BOOL includeSystem);

CK_C_VISIBLE_PUBLIC HCkZipEntry
CkZip_AppendHex(HCkZip cHandle, const char *fileName, const char *encodedCompressedData);
...
CK_C_VISIBLE_PUBLIC HCkTask
CkZip_AppendOneFileOrDirAsync(HCkZip cHandle, const char *fileOrDirPath, BOOL saveExtraPath);

CK_C_VISIBLE_PUBLIC HCkZipEntry
CkZip_AppendString(HCkZip cHandle, const char *internalZipFilepath, const char *textData);

CK_C_VISIBLE_PUBLIC HCkZipEntry
CkZip_AppendString2(HCkZip cHandle, const char *internalZipFilepath, const char *textData, const char *charset);
...
CK_C_VISIBLE_PUBLIC BOOL
CkZip_ReplaceEmbedded(HCkZip cHandle, const char *exeFilename, const char *resourceName, const char *zipFilename);
...
CK_C_VISIBLE_PUBLIC HCkTask
CkZip_UnzipMatchingAsync(HCkZip cHandle, const char *dirPath, const char *pattern, BOOL verbose);

CK_C_VISIBLE_PUBLIC int CkZip_UnzipMatchingInto(HCkZip cHandle, const char *dirPath, const char *pattern, BOOL verbose);

CK_C_VISIBLE_PUBLIC HCkTask
CkZip_UnzipMatchingIntoAsync(HCkZip cHandle, const char *dirPath, const char *pattern, BOOL verbose);
...
CK_C_VISIBLE_PUBLIC BOOL
CkZip_WriteExe2(HCkZip cHandle, const char *exePath, const char *destExePath, BOOL bAesEncrypt, int keyLength,
                const char *password);
